<script lang="ts">
	...
	// URLパラメータから現在のドキュメントIDを取得
	let selectedDocId = $state<number | null>(data.currentDocId);
	...
	// ドキュメントが変更されたらselectedDocIdを更新
	$effect(() => {
		if (data.currentDocId !== selectedDocId) {
			selectedDocId = data.currentDocId;
		}
	});
...
<svelte:head>
	<title>{data.board.title} - ドキュメント</title>
</svelte:head>
