...
<svelte:head>
	<title>{data.document.title} - {data.project.title}</title>
</svelte:head>

<div class="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-100">
	<!-- Header -->
	<header class="bg-white shadow-sm border-b border-emerald-200 sticky top-0 z-10">
		<div class="max-w-7xl mx-auto px-6 py-4">
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-4">
					<a
						href="/dashboard/projects/{data.project.id}"
						class="p-2 text-gray-600 hover:bg-emerald-50 rounded-lg transition-colors"
						title="プロジェクトに戻る"
					>
						<ArrowLeft size={20} />
					</a>
					<div>
						...
						<p class="text-sm text-gray-500">{data.project.title}</p>
					</div>
				</div>
				...
				<div class="flex items-center gap-4">
					...
					>
						...
						<button
							type="submit"
							disabled={saving}
							class="px-6 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors flex items-center gap-2 shadow-sm disabled:opacity-50 text-base"
						>
							<Save size={18} />
							{saving ? '保存中...' : '保存'}
						</button>
	...
	<main class="max-w-5xl mx-auto px-6 py-8">
		...
		<div class="mb-6">
			...
			/>
			<div class="h-px bg-gradient-to-r from-emerald-200 via-emerald-300 to-emerald-200 mt-2"></div>
		</div>

		<!-- Formatting Toolbar -->
		<div class="bg-white rounded-t-xl shadow-sm border border-emerald-200 border-b-0 p-3">
			<div class="flex flex-wrap gap-2">
				<button
					type="button"
					onclick={() => execCommand('bold')}
					class="p-2.5 text-gray-700 hover:bg-emerald-50 hover:text-emerald-600 rounded-lg transition-colors border border-emerald-200"
					title="太字"
				>
					<Bold size={18} />
				</button>
				...
				<button
					...
					class="px-3 py-2.5 text-gray-700 hover:bg-emerald-50 hover:text-emerald-600 rounded-lg transition-colors border border-emerald-200 font-bold text-base"
					title="見出し1"
				>
					H1
				</button>
		...
		<!-- Editor Content -->
		<div class="bg-white rounded-b-xl shadow-sm border border-emerald-200 p-8">
			<div
				bind:this={editorRef}
		...
		<div class="mt-8 bg-emerald-50 rounded-xl p-6 border border-emerald-200">
			<h3 class="text-base font-semibold text-emerald-900 mb-3">ヒント</h3>
