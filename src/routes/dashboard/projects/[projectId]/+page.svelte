<script lang="ts">
	...
	let activeTab = $state<'boards' | 'documents' | 'sns'>('boards');
		...
		<div class="bg-white rounded-xl shadow-md overflow-hidden mb-6">
			...
			<div class="p-6 flex gap-6">
				{#if activeTab === 'boards'}
					...
					<div class="flex-1">
						<div class="flex items-center justify-between mb-4">
							<h3 class="text-lg font-semibold text-gray-800">このプロジェクトのボード</h3>
				...
				<aside class="w-80 bg-white rounded-xl shadow-md p-4 h-fit sticky top-6">
					...
					{#if data.allBoards && data.allBoards.length > 0}
						<div class="space-y-2">
							{#each data.allBoards as board}
								<a
									href="/dashboard/projects/{board.project_id}?section=boards&board={board.id}"
								...
								>
									<div class="flex items-start justify-between mb-1">
										<h4 class="font-semibold text-gray-800 text-xs truncate flex-1">{board.title}</h4>
										<ExternalLink size={12} class="text-indigo-600 flex-shrink-0 ml-2" />
									</div>
					...
					{:else}
						<p class="text-gray-500 text-xs text-center py-4">ボードがありません</p>
					{/if}
				</aside>
					...
					{#if data.allDocuments && data.allDocuments.length > 0}
						<div class="space-y-2">
							{#each data.allDocuments as doc}
								...
								>
									<div class="flex items-start justify-between mb-1">
										<h4 class="font-semibold text-gray-800 text-xs truncate flex-1">{doc.title}</h4>
										<ExternalLink size={12} class="text-purple-600 flex-shrink-0 ml-2" />
									</div>
									<p class="text-xs text-gray-600 mb-1">{doc.project_title || 'プロジェクト'}</p>
									<p class="text-xs text-gray-500">
										{new Date(doc.updated_at).toLocaleDateString('ja-JP', { month: 'short', day: 'numeric' })}
									</p>
								</a>
							{/each}
						</div>
					{:else}
						<p class="text-gray-500 text-xs text-center py-4">ドキュメントがありません</p>
					{/if}
				</aside>
			{:else if activeTab === 'sns'}
				...
				<div class="flex-1">
					<div class="bg-white rounded-xl shadow-md overflow-hidden">
						...
						<div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<!-- Twitter/X連携 (実装前) -->
							<div class="p-4 bg-blue-50 rounded-lg border border-blue-100 opacity-60">
								<div class="flex items-center justify-between mb-2">
									<div class="flex items-center gap-2">
										...
										<span class="font-medium text-blue-800">Twitter/X</span>
									</div>
									<span class="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">実装前</span>
								</div>
								<p class="text-sm text-gray-600">フォロワー数・エンゲージメント分析</p>
							</div>

							<!-- Instagram連携 (実装前) -->
							<div class="p-4 bg-pink-50 rounded-lg border border-pink-100 opacity-60">
								<div class="flex items-center justify-between mb-2">
									<div class="flex items-center gap-2">
								...
								<p class="text-sm text-gray-600">フォロワー数・投稿分析</p>
							</div>

							<!-- その他のSNS (実装前) -->
							<div class="p-4 bg-purple-50 rounded-lg border border-purple-100 opacity-60">
								<div class="flex items-center justify-between mb-2">
									<div class="flex items-center gap-2">
										<Users size={20} class="text-purple-600" />
										<span class="font-medium text-purple-800">その他SNS</span>
									</div>
									<span class="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">実装前</span>
								</div>
								<p class="text-sm text-gray-600">TikTok、Facebook、など</p>
							</div>
						</div>
					</div>
				</div>
				...
				<aside class="w-80 bg-white rounded-xl shadow-md p-4 h-fit sticky top-6">
					...
					<div class="space-y-3">
						<!-- YouTube統計 -->
						<div class="p-3 bg-gradient-to-br from-red-50 to-orange-50 rounded-lg border border-red-200">
							<div class="flex items-center gap-2 mb-2">
								<Youtube size={16} class="text-red-600" />
								<span class="text-xs font-semibold text-gray-800">YouTube</span>
							</div>
							<div class="space-y-1">
								...
								<div class="flex justify-between text-xs">
									<span class="text-gray-600">総再生回数</span>
									<span class="font-semibold text-gray-800">-</span>
								</div>
							</div>
						</div>
						...
						<!-- その他のSNS統計 (実装前) -->
						<div class="p-3 bg-gray-50 rounded-lg border border-gray-200 opacity-60">
							<p class="text-xs text-gray-500 text-center">その他のSNS統計は実装予定</p>
						</div>
					</div>
				</aside>
			{/if}
